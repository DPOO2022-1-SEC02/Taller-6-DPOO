...
public class InterfazLibreria extends JFrame
{
	...
	public void cargarArchivos(File archivo_categorias, File archivo_libros)
	{
		try
		{
			...
			ArrayList<String> creadas;
			creadas = libreria.darCreadas();
			String textocreadas = "No se crearon nuevas categorias";
			if (creadas.size() != 0) {
				textocreadas = "Las categorias creadas fueron: \n";
				for (String categoria: creadas) {
					Categoria laCategoria = libreria.buscarCategoria(categoria);
					int size = laCategoria.contarLibrosEnCategoria();
					textocreadas = textocreadas + categoria + " - " + size +" libro(s)"+"\n";
				}
			
			JOptionPane.showMessageDialog(this, textocreadas);
			}
	...
	{
		try {
			String viejisimo = JOptionPane.showInputDialog(this, "Escriba el nombre de la categoria a renombrar", "Categoria a renombrar");
			...
			JOptionPane.showMessageDialog(this, "Se renombro correctamente la categoria", "Renombrar Exitoso", JOptionPane.INFORMATION_MESSAGE);
		}
		catch (Exception e)
		{
			JOptionPane.showMessageDialog(this, "No se pudo renombrar la categoria", "No se pudo renombrar", JOptionPane.WARNING_MESSAGE);
